...
export default function HeroSection() {
  ...
  const [user, setUser] = useState(null);  // Stato per tenere traccia dell'utente autenticato
  ...
  return (
    <>
        ...
        >
          ...
          {user ? (
            <div className="text-center">
              <div className="w-12 h-12 mb-4 bg-blue-700 rounded-full flex justify-center items-center mx-auto transform transition-transform duration-300 hover:scale-110 cursor-pointer shadow-lg" onClick={() => setShowModal(true)}>
                <span className="text-3xl text-white">+</span>
              </div>
              <h2 className="text-lg md:text-xl font-bold mb-2" style={{ fontWeight: 600 }}>
            ...
            <div className="text-center">
              <h2 className="text-lg md:text-xl font-bold mb-2" style={{ fontWeight: 600 }}>
                Accedi per iscriverti al convegno
              </h2>
              <button onClick={() => router.push('/login')} className="bg-blue-500 text-white py-2 px-4 rounded-md">
                Login
              </button>
            </div>
          )}
      ...
      {/* Modal del form con stile migliorato, visibile solo per utenti autenticati */}
      {showModal && user && (
